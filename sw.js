/* Service Worker – תמלול */
const VERSION = 'tamlul-v1.0.0';
const SHELL = ['./', './index.html', './manifest.webmanifest', './icon-192.png', './icon-512.png', './icon-maskable-512.png', './apple-touch-icon.png'];
const SHARE_CACHE = 'tamlul-share';
const LIB_CACHE = 'tamlul-libs';
...
self.addEventListener('activate', (e) => {
  e.waitUntil((async () => {
    ...
    await Promise.all(keys.filter((k) => k !== VERSION && k !== SHARE_CACHE && k !== LIB_CACHE).map((k) => caches.delete(k)));
